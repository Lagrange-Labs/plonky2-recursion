    ...
    [(); C::Hasher::HASH_SIZE]:,
{
    fn build_circuit(config: CircuitConfig) -> Self {
        ...
        let (pt, vt) = (
            dummy_circuit_builder.add_virtual_proof_with_pis(&fake_circuit.common),
    ...
    [(); C::Hasher::HASH_SIZE]:,
{
    pub(crate) fn build_merge_circuit(
        ...
        let (proof_targets, inner_data): (Vec<_>, Vec<_>) = (0..num_proofs_to_merge)
            .map(|_| {
                (
                    builder.add_virtual_proof_with_pis(&rec_data),
                    VerifierCircuitTarget {
    ...
    pub(crate) fn merge_proofs<'a>(
        ...
        let dummy_proof = if let Some(pt) = input_proofs_iterator.next() {
            tmp_dummy_proof = self.dummy_circuit.generate_dummy_proof(
                &previous_proof.public_inputs[..PI::num_public_inputs()],
                &self.circuit_set,
            )?;
...
mod test {
        ...
        [(); C::Hasher::HASH_SIZE]:,
    {
            ...
            [(); C::Hasher::HASH_SIZE]:,
        {
            ...
            let mut aggregated_proof = self.merge_proofs(
                ...
                verifier_data[..num_proofs_to_be_aggregated].iter().cloned(),
            )?;
            ...
            let merge_next_chunk = |chunk_index, accum, proof_chunk, vd_chunk| {
                ...
                let proof = self.merge_proofs(&proofs, merge_vd.into_iter()).unwrap();
        ...
        fn check_proof(&self, proof: &ProofWithPublicInputs<F, C, D>) -> Result<()>
        where
            [(); C::Hasher::HASH_SIZE]:,
        {
            ...
            assert_eq!(
                PI::num_public_inputs()
                    + num_targets_for_circuit_set::<F, D>(self.circuit_data.common.config.clone()),
                num_public_inputs
            );
            ...
            for i in 0..num_public_inputs {
                ...
                self.aggregated_proof_circuit_data()
                    .verify(proof)
                    .unwrap_err();
            }
            ...
            assert_eq!(
                RECURSION_THRESHOLD,
                self.aggregated_proof_circuit_data().common.degree_bits()
            );
        ...
        [(); C::Hasher::HASH_SIZE]:,
    {
        fn build_circuits(
            ...
            let mul_base_circuit =
                MulBaseCircuit::<F, C, D>::build_base_circuit(&config, RECURSION_THRESHOLD, false);
        ...
        {
            self.merge_circuit
                .aggregate_proofs(proofs, verifier_data, aggregation_factor)
        }
    }
    ...
    fn test_wrong_public_inputs(
        ...
        let (mut base_proofs, verifier_data, _state) =
            circuits.generate_base_proofs(NUM_PROOFS, init, Some(test_case.clone()));
    ...
    fn test_add_dummy_proofs(circuits: &Circuits<F, PC, D, SimpleStatePublicInput>) {
        ...
        let dummy_proof = circuits
            ...
            .generate_dummy_proof(
                &base_proofs.last().unwrap().public_inputs
                    [..SimpleStatePublicInput::num_public_inputs()],
    ...
    struct PublicInputAccumulatorBaseCircuit<
        F: RichField + Extendable<D>,
        C: GenericConfig<D, F = F>,
        const D: usize,
    > {
    ...
    impl<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>
        PublicInputAccumulatorBaseCircuit<F, C, D>
    where
        C::Hasher: AlgebraicHasher<F>,
    {
        fn build_base_circuit(config: CircuitConfig) -> Self {
            ...
            let private_input_values: [Target; 2] =
                builder.add_virtual_targets(2).try_into().unwrap();
            let input_hash =
                builder.hash_n_to_hash_no_pad::<C::Hasher>(private_input_values.to_vec());
        ...
        [(); C::Hasher::HASH_SIZE]:,
    {
        fn build_circuits(config: CircuitConfig) -> Self {
            let base_circuit =
                PublicInputAccumulatorBaseCircuit::<F, C, D>::build_base_circuit(config.clone());
            ...
            let merge_circuit =
                MergeCircuit::<F, C, D, PublicInputAccumulator>::build_merge_circuit(
                    config,
                    AGGREGATION_FACTOR,
                    vec![
                        wrap_circuit
            ...
            assert_eq!(
                wrap_circuit.final_proof_circuit_data().common,
                merge_circuit.aggregated_proof_circuit_data().common
            );
    ...
    #[fixture]
    #[once]
    fn circuits_for_public_input_accumulator(
        _logger: (),
    ...
    fn test_merge_circuit_with_conditional_input_aggregation(
        ...
        let num_proofs = 8;
        let base_proofs = circuits_for_public_input_accumulator
            .generate_base_proofs(num_proofs)
            .unwrap();

        let aggregated_proof = circuits_for_public_input_accumulator
            .merge_circuit
            .aggregate_proofs(
                &base_proofs[..num_proofs - 1],
                vec![
                    &circuits_for_public_input_accumulator
                        .wrap_circuit
                        .final_proof_circuit_data()
                        .verifier_only;
                    num_proofs - 1
        ...
        let mut input_accumulator =
            aggregated_proof.public_inputs[..num_public_inputs / 2].to_vec();
        let mut output_accumulator =
            aggregated_proof.public_inputs[num_public_inputs / 2..num_public_inputs].to_vec();

        input_accumulator
            .extend_from_slice(&base_proofs[num_proofs - 1].public_inputs[..num_public_inputs / 2]);
        output_accumulator.extend_from_slice(
            &base_proofs[num_proofs - 1].public_inputs[num_public_inputs / 2..num_public_inputs],
        );
        ...
        let dummy_proof = circuits_for_public_input_accumulator
            ...
            .generate_dummy_proof(
                &base_proofs.last().unwrap().public_inputs
                    [..PublicInputAccumulator::num_public_inputs()],
                &circuits_for_public_input_accumulator
                    .merge_circuit
                    .circuit_set,
            )
        ...
        let final_aggregated_proof = circuits_for_public_input_accumulator
            .merge_circuit
            .aggregate_proofs(
                vec![
                    aggregated_proof,
                    base_proofs[num_proofs - 1].clone(),
                    dummy_proof,
                ]
                .as_slice(),
                vec![
    ...
    fn test_aggregation_of_dummy_proofs_only(
        ...
        let dummy_proofs = (0..2)
            .map(|_| {
                // generate random values as public inputs
                let input_values = (0..PublicInputAccumulator::num_public_inputs())
                    .map(|_| F::rand())
                    .collect::<Vec<_>>();
                circuits_for_public_input_accumulator
                    .merge_circuit
    ...
    fn test_aggregation_of_real_proof_with_dummy_proof(
        ...
        let real_proof = {
            let proof = circuits_for_public_input_accumulator
                .base_circuit
                .generate_base_proof([F::rand(), F::rand()])
                .unwrap();
            circuits_for_public_input_accumulator
                .wrap_circuit
                .wrap_proof(
                    proof,
                    CircuitSetDigest::from(
                        &circuits_for_public_input_accumulator
                            .merge_circuit
                            .circuit_set,
                    ),
