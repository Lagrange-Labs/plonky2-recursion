...
pub fn prepare_base_circuit_for_circuit_set<
    'a,
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F>,
    const D: usize,
...
mod test_circuits {
        ...
        [(); C::Hasher::HASH_SIZE]:,
    {
        ...
        pub(crate) fn generate_base_proof(
            ...
            let res = (0..self.num_powers).fold(init, |acc, _| {
        ...
        [(); C::Hasher::HASH_SIZE]:,
    {
        pub(crate) fn build_circuit(config: &CircuitConfig, num_leaves: usize) -> Self {
            ...
            assert!(
                full_tree_height >= CAP_HEIGHT,
                "CAP_HEIGHT={} for MerkleRootStateBaseCircuit is \
            too high: it should be no greater than ceil(log2(num_leaves)) = {}",
                CAP_HEIGHT,
                full_tree_height
            );
