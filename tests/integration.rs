...
use generic_recursion::recursion::{
    build_verifier_circuit_data, prepare_base_circuit_for_circuit_set, BaseCircuitInfo,
    PreparedProof, RecursionCircuit,
};
...
impl<const NUM_EL: usize> FromIterator<Target> for VectorState<NUM_EL> {
    fn from_iter<T: IntoIterator<Item = Target>>(iter: T) -> Self {
        ...
        VectorState(state.try_into().unwrap())
    }
}
    ...
    fn try_from(targets: &[Target]) -> std::result::Result<Self, Self::Error> {
        if targets.len() != Self::num_targets() {
            Err(anyhow::Error::msg(format!(
                "expected {} targets to build VectorState, found {}",
                Self::num_targets(),
                targets.len()
            )))
        } else {
...
struct BaseCircuit<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize> {
    ...
    BaseCircuit<F, C, D>
{
        ...
        C::Hasher: AlgebraicHasher<F>,
    {
        ...
        for (&input_t, &intermediate_t) in input_state_targets
            .iter()
            .zip(intermediate_state_targets.iter())
        {
        ...
        for i in 0..NUM_ROUNDS {
            intermediate_state_targets = intermediate_state_targets
                .iter()
                .zip(private_input_targets.iter())
                .map(|(&intermediate_t, &input_t)| {
                    if i % 2 == 0 {
            ...
            intermediate_state_targets =
                builder.hash_n_to_m_no_pad::<C::Hasher>(intermediate_state_targets, STATE_LEN);
        }

        input_state_targets.extend_from_slice(intermediate_state_targets.as_slice());

        let public_input_targets =
            VectorStatePublicInput::try_from_public_input_targets(input_state_targets.as_slice())
                .unwrap();
...
// input shared state
struct BaseCircuitU32<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize> {
    public_input: VectorStatePublicInput<STATE_LEN>,
    ...
    BaseCircuitU32<F, C, D>
{
    ...
    fn truncate_target_to_u32(builder: &mut CircuitBuilder<F, D>, target: Target) -> Target {
        ...
        let least_significant_limbs = &builder
            .split_le_base::<B>(target, (64 + B_BITS - 1) / B_BITS)[..(32 + B_BITS - 1) / B_BITS];
        ...
        C::Hasher: AlgebraicHasher<F>,
    {
        ...
        let mut intermediate_state_targets = input_state_targets
            .iter()
            .map(|&input_t| {
                let target = Self::truncate_target_to_u32(&mut builder, input_t);
                range_check_u32_circuit::<F, D>(&mut builder, vec![U32Target(target)]);
                target
            })
            .collect::<Vec<_>>();
        ...
        for i in 0..NUM_ROUNDS {
            intermediate_state_targets = intermediate_state_targets
                ...
                .collect::<Vec<_>>();
            intermediate_state_targets = builder
                .hash_n_to_m_no_pad::<C::Hasher>(intermediate_state_targets, STATE_LEN)
                .iter()
                .map(|&target| Self::truncate_target_to_u32(&mut builder, target))
                .collect();
        }
    ...
    let base_proofs = (0..NUM_PROOFS)
        .map(|i| {
            let (proof, vd) = if rng.gen() {
                (base_circuit.generate_base_proof(state).unwrap(), &base_vd)
            } else {
                (
            ...
            state = proof.public_inputs[STATE_LEN..]
                .to_vec()
                .try_into()
                .unwrap();
